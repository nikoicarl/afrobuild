...
module.exports = (socket, Database) => {
    socket.on('create_service', async (browserblob) => {
        const {
            name,
            price,
            category,
        ...
        try {
            if (md5(userid) === melody2) {
                ...
                const serviceId = isNew ? 0 : service_hiddenid;
                ...
                if (result && result.affectedRows !== undefined) {
                    ...
                    return socket.emit(`${melody1}_create_service`, {
                        success: true,
                        message: `Service has been ${isNew ? 'created' : 'updated'} successfully`
                    });
                }
