...
function renderReportContainer(targetId = 'afrobuild_product_report_page_form_display') {
...
function renderReportForm({
    formClass = 'afrobuild_product_report_form',
    dropdownClass = 'afrobuild_product_report_dropdown',
    dateRangeClass = 'afrobuild_product_report_date_range',
    dateInputClass = 'afrobuild_product_report_date',
    submitBtnClass = 'afrobuild_product_report_form_btn',
    displayContainerClass = 'afrobuild_product_report_display_page'
} = {}) {
    return `
        <div class="row">
...
// Renders the product report
function ProductReportPage(data) {
    const logo = `
    ...
    const htmlTable = data.data.length > 0
        ? data.data.map(item => `
            <tr>
                ...
                <td>${Number(item.price).toLocaleString('en-GH', {
        ...
        <div class="stat-card shadow-sm border rounded-lg p-4 mb-5 bg-white">
            <div id="afrobuild_report_print_div">
                ...
                <div class="d-flex justify-content-between flex-wrap align-items-start mb-4">
                    <div>
                        <h4 class="text-dark mb-1">Product Report</h4>
                        <p class="text-muted" style="font-size: 14px;">Detailed product summary</p>
                    </div>
                    <div class="text-end">
                ...
                <div class="table-responsive">
                    <table id="product_table" class="table table-bordered table-striped table-hover" style="font-size: 13px;">
                ...
                <div class="card mt-4 shadow-sm border no-print">
                    <div class="card-body d-flex flex-wrap justify-content-between align-items-center gap-3">
                        ...
                        <div class="d-flex flex-wrap gap-2">
                            ...
                            <button type="button" class="btn btn-outline-info" onclick="exportHTMLTableToCSV('product_table', 'product_report.csv')">
                                <i class="icon-file-text me-1"></i> CSV
                            </button>
                            <button type="button" class="btn btn-outline-primary" onclick="exportHTMLTableToExcel('product_table', 'product_report.xlsx')">
...
(() => {
    ...
    $('#afrobuild_product_report_page_form_display').html(formHtml);

    if (typeof addPageScript === 'function') {
        addPageScript('app/productReportAjax');
    } else {
